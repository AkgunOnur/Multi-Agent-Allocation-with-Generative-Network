...
class TrainingConfig():
    ...
    def init_env_name(self, game_name, map_idx):
        # env_name = game_name
        ...
        self.project_name = f'MiniGrid_{game_name}_map_{str(map_idx)}'

    

    def init_logger(self):
        if self.group_name is not None:
            wandb.login(key="89e2bcd019ca8663762481724f70ce99677af029")
            self.wandb_run = wandb.init(
                name=self.project_name,
                # entity='MiniGrid_Deir',  # your project name on wandb
                project= 'MiniGrid_Deir', #self.project_name, #self.project_name,
        ...
        self.log_dir = os.path.join(self.log_dir, self.project_name, self.start_datetime, str(self.run_id))
    ...
    def get_minigrid_venv(self, wrapper_class=None):
        venv = make_vec_env(
            self.env_name,
            wrapper_class=wrapper_class,
            vec_env_cls=CustomSubprocVecEnv,
    ...
    def get_callbacks(self, train_env, map_idx):
        
        if self.group_name is not None:
            # wandb.init()
            callbacks = CallbackList([
                WandbCallback(gradient_save_freq=50,verbose=1)])
                # EvalCallback(eval_env=train_env, n_eval_episodes = 3, eval_freq = 100,
                #                         best_model_save_path = "saved_models/" + self.game_name + "/map_" + str(map_idx), 
