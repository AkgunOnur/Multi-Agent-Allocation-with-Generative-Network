...
def train(flags, target_map, map_index):
    if flags.xpid is None:
        # flags.xpid = 'cbet-%s' % (time.strftime('%Y%m%d-%H%M%S'))
        flags.xpid = flags.env.split('-')[1] + "_map_" + str(map_index)
    ...
    models_and_states = init_models_and_states(flags, target_map)
    ...
    def batch_and_learn(i, lock=threading.Lock(), terminate_flag=mp.Event()):
        """Thread target for the learning process."""
        nonlocal frames, stats
        timings = prof.Timings()
        while frames < flags.total_frames and not terminate_flag.is_set(): 
            timings.reset()
        ...
        while frames < flags.total_frames:
            ...
            if stats.get('episode_returns', None):
                mean_return = 'Return per episode: %.1f. ' % stats[
                    'mean_episode_return']
                # print ("\nMean Return: ", stats['mean_episode_return'])
            else:
                mean_return = ''
            ...
            # if not stats.get('mean_rewards', None):
            #     print ("\nMean Reward: ", stats["mean_rewards"])
            #     print ("\n")
            
            # if frames % flags.eval_freq == 0:
            #     print ("Evaluation mode!")
            #     keys = ['reward', 'episode_return', 'episode_step', 'episode_win']
            #     stats = test_model(actor_model, keys, flags, target_map)
            #     mean_reward = np.mean(stats["reward"])
            #     print ("mean reward: ", mean_reward)
            #     print ("mean episode return: ", np.mean(stats['episode_return']))
