...
class PPORollout(BaseAlgorithm):
    ...
    def on_training_start(self):
        if isinstance(self._last_obs, Dict):
            self._last_obs = self._last_obs["image"]
    ...
    def learn(
        ...
        total_timesteps, callback = self._setup_learn(
            total_timesteps=total_timesteps, eval_env=None, callback=callback, eval_freq=eval_freq, n_eval_episodes=n_eval_episodes, 
            log_path = eval_log_path, reset_num_timesteps = reset_num_timesteps, tb_log_name = tb_log_name
        )
        ...
        best_eval_reward = 0 
        model_folder = os.path.join(eval_log_path, "best_model")
        while self.num_timesteps < total_timesteps:
            ...
            if self.num_timesteps % eval_freq == 0:  # assuming eval_freq is the frequency of evaluation
                eval_reward = self.evaluate_agent(eval_env, n_eval_episodes)  # you might need to write this function
                print (f"Evaluation mode! Reward: {eval_reward:.6}")

                if eval_reward > best_eval_reward:
                    best_eval_reward = eval_reward
                    policy_state_dict = self.policy.state_dict()
                    th.save(policy_state_dict, model_folder)
    ...
    def test(
        ...
        eval_reward = self.evaluate_agent(eval_env, n_eval_episodes, best_model=model_folder)  # you might need to write this function
        print (f"Evaluation mode! Reward: {eval_reward:.6}")
            
            
    # This is an example evaluation function
    def evaluate_agent(self, eval_env, n_eval_episodes, best_model = None):
        total_rewards = 0
        eval_iter = 0
        if best_model is not None:
            print ("Best model is loaded!")
            state_dict = th.load(best_model)
        ...
        for _ in range(n_eval_episodes):
            ...
            _last_policy_mems = self._last_policy_mems
            obs_tensor = obs_as_tensor(self._last_obs, self.device)
            # print ("obs: ", obs_tensor.size())
            while not done and eval_iter < 500 and episode_reward < 0.5:
                eval_iter += 1
                # print ("obs: ", obs_tensor.size())
                actions, values, log_probs, policy_mems = self.policy.forward(obs_tensor, _last_policy_mems)
                actions = actions.cpu().numpy()
                ...
                # Transition
                new_obs, rewards, dones, infos = eval_env.step(clipped_actions)
                if isinstance(new_obs, Dict):
                    new_obs = new_obs["image"]


                env_idx = np.argmax(rewards)
                episode_reward += rewards[env_idx]
                done = dones[env_idx]
                _last_obs = new_obs
                ...
                if policy_mems is not None:
                    _last_policy_mems = policy_mems.detach().clone()

                # print ("dones: ", dones)
                # print ("rewards: ", rewards)
                # print ("episode_reward: ", episode_reward, " done: ", done)
            total_rewards += episode_reward

        return total_rewards / n_eval_episodes