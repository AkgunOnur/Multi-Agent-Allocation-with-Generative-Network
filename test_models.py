...
def test_model(model, keys, flags, target_map, n_episodes=2):
    torch.manual_seed(flags.seed)
    torch.cuda.manual_seed(flags.seed)
    np.random.seed(flags.seed)
    verbose = False

    env = make_environment(flags, target_map)
    ...
    for episode in tqdm(range(n_episodes)):
        ...
        for step in tqdm(range(1000), leave=False, disable=not verbose):
            with torch.no_grad():
