...
# from src.utils.env_util_bayes import make_vec_env, make_atari_env
from stable_baselines3.common.vec_env import SubprocVecEnv, DummyVecEnv
from stable_baselines3.common.env_util import make_vec_env
# from stable_baselines3.common.env_util import make_vec_env, make_atari_env
# from src.utils.env_util_bayes import make_atari_env, make_vec_env
from src.utils.env_utils import ImgObsWrapper

    

def train(config, map_idx, current_map):
    th.autograd.set_detect_anomaly(False)
    th.set_default_dtype(th.float32)
    th.backends.cudnn.benchmark = False

    # current_folder = config.game_name + "_map_" + str(map_idx)

    env_kwargs = {"size": current_map.shape[0] + 2, "env_map": current_map, "visualization": False}  #"env_map": current_map, , "num_cols": 3, "num_rows": 3


    train_env = make_vec_env(env_id=config.env_id, seed=5, n_envs=config.num_processes, wrapper_class=ImgObsWrapper, env_kwargs=env_kwargs, vec_env_cls=DummyVecEnv)

    eval_env = make_vec_env(env_id=config.env_id, seed=5, n_envs=config.num_processes, wrapper_class=ImgObsWrapper, env_kwargs=env_kwargs, vec_env_cls=DummyVecEnv)

    callbacks = config.get_callbacks(train_env, map_idx)
    ...
    best_model_path = "saved_models/" + config.game_name + "/map_" + str(map_idx)

    if not os.path.exists(best_model_path):
        # If it doesn't exist, create it
        os.makedirs(best_model_path)

    reward_list = [0.95, 0.95, 0.9, 0.9, 0.8, 0.8]
    reward_threshold = reward_list[map_idx]
...
@click.command()
# Training params
@click.option('--env_id', default='MiniGrid-CustomDoorKey-v0', type=str, help='Directory for saving training logs') # MiniGrid-CustomKeyCorridor-v0, MiniGrid-CustomDoorKey-v0
@click.option('--map_file', default='doorkey_all_maps.pickle', type=str, help='Directory for saving training logs') # doorkey_all_maps, keycorridor_all_maps
@click.option('--run_id', default=0, type=int, help='Index (and seed) of the current run') 
@click.option('--group_name', default="5Aralik_DoorKey", type=str, help='Group name (wandb option), leave blank if not logging with wandb') # "31Ekim_KeyCorridor", "2Kasim_KeyCorridor"
@click.option('--log_dir', default='./logs_doorkey_5Aralik', type=str, help='Directory for saving training logs')
@click.option('--game_name', default="DoorKey", type=str, help='e.g. DoorKey-8x8, ninja, jumper') # KeyCorridor, DoorKey
@click.option('--total_steps', default=int(1e9), type=int, help='Total number of frames to run for training')
...
def main(
    env_id, map_file, run_id, group_name, log_dir, total_steps, features_dim, model_features_dim, learning_rate, model_learning_rate,
    ...
    map_idx = 3
    target_map = new_target_maps[map_idx]
    # for map_idx, target_map in enumerate(new_target_maps): 
    ...
    print ("Current Map: \n", target_map)     
    config.init_env_name(game_name, map_idx)
    config.init_meta_info()
    config.init_logger()
    config.init_values()

    train(config, map_idx, target_map)
