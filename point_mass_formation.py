...
class AgentFormation(gym.Env):
    ...
    def get_observation(self):
        ...
        self.free_area = np.zeros((self.out_shape, self.out_shape))
        self.observation = np.zeros((3,self.out_shape, self.out_shape))
        ...
        for prize_loc in self.prize_locations:
            self.prize_map[prize_loc[0],prize_loc[1]] = 1
        ...
        self.observation[0,:,:] = np.copy(self.free_area)
    ...
    def reset(self, level, index = None):
        ...
        x_list = np.arange(1, 4, self.grid_res)
        y_list = np.arange(1, 4, self.grid_res)
        self.init_list = []
        ...
        self.N_prize = np.random.randint(1, 6)
        # self.N_prize = 40
