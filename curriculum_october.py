...
from stable_baselines3.common.callbacks import EvalCallback, StopTrainingOnRewardThreshold, StopTrainingOnNoRemarkableImprovement
...
def bayes_curriculum_october(args):
    ...
    for j_map, target_map in enumerate(new_target_maps):
        for i_iter in range(args.N_train_iter):
            ...
            while(train_is_on):
                ...
                # similarity_to_target_euc = calculate_euclidian_distances(current_map, target_map, distance_coef)
                # similarity_to_target_index = calculate_map_items_difference_target(map_items, target_items)
                # similarity_rates.append(similarity_to_target_euc + similarity_to_target_index)
                ...
                env_kwargs = {"env_map": current_map, "see_through_walls": True}
                ...
                train_env = make_vec_env(env_id=args.env_id, n_envs=args.n_procs, wrapper_class=ImgObsWrapper, env_kwargs=env_kwargs, monitor_dir=current_folder + "/level_" + str(level), vec_env_cls=DummyVecEnv)  
                train_env.reset()
                ...
                if level > 0:
                    ...
                    model = args.algorithm(args.policy, train_env, policy_kwargs=policy_kwargs, tensorboard_log= current_folder + "/level_" + str(level) + "/tensorboard/")
                ...
                callback = EvalCallback(eval_env=train_env, callback_after_eval=stop_callback, n_eval_episodes = args.n_eval_episodes, eval_freq = N_eval_freq, log_path = current_folder + "/level_" + str(level) +"_log",
                ...
                if target_finetuning:
                    ...
                    env_kwargs = {"env_map": target_map, "see_through_walls": True}
                    # train_env = make_atari_env(env_id=args.env_id, n_envs=args.n_procs, env_kwargs=env_kwargs, monitor_dir=current_folder + "/level_" + str(level) , vec_env_cls=DummyVecEnv)
                    target_env = make_vec_env(env_id=args.env_id, n_envs=args.n_procs, wrapper_class=ImgObsWrapper, env_kwargs=env_kwargs, monitor_dir=current_folder, vec_env_cls=DummyVecEnv)  
...
def vanilla_train(args):
    ...
    for j_map, env_map in enumerate(new_target_maps[map_offset:]):
        for i_iter in range(args.N_train_iter):
    
            print ("Current Iter #" + str(i_iter + iter_offset) +" Env. Map #" + str(j_map + map_offset) + "\n")
            print (env_map)

            current_folder = args.output_folder + "_iter_" + str(i_iter + iter_offset) + "_map_" + str(j_map + map_offset)
            if not os.path.exists(current_folder):
                os.makedirs(current_folder)

            # env_kwargs = {"visualization": args.visualization, "env_map": env_map}    # , "custom": False, "env_map": None
            env_kwargs = {"env_map": env_map, "see_through_walls": True,  "render_mode": "human"}
            ...
            train_env = make_vec_env(env_id=args.env_id, n_envs=args.n_procs, wrapper_class=ImgObsWrapper, env_kwargs=env_kwargs, monitor_dir=current_folder, vec_env_cls=DummyVecEnv)  
            train_env.reset()
            
            model = args.algorithm(args.policy, train_env, policy_kwargs=policy_kwargs, tensorboard_log= current_folder + "/tensorboard/")
